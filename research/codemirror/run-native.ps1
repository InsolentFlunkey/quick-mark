...
$outputDirectory = Join-Path $PSScriptRoot 'baseline'
...
$previousOutput = $env:QUICKMARK_BENCH_OUTPUT
$env:QUICKMARK_BENCH_OUTPUT = $eventsPath
$benchmark = $null
try {
  $executable = Join-Path $projectRoot 'src-tauri/target/debug/quick-mark.exe'
  $benchmark = Start-Process -FilePath $executable -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru
  ...
  while (-not $benchmark.HasExited) {
    ...
    if (((Get-Date) - $lastProgress).TotalSeconds -gt $WatchdogSeconds) { throw "No benchmark checkpoint for $WatchdogSeconds seconds." }
  ...
  $env:QUICKMARK_BENCH_OUTPUT = $previousOutput
  if ($benchmark -and -not $benchmark.HasExited) { $benchmark.Kill(); $benchmark.WaitForExit() }
}
